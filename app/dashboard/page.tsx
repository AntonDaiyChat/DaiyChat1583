  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          onClick={() => router.push('/calendar')}
          className="flex aspect-square flex-col justify-between rounded-2xl border border-white/10 bg-white/5 p-4 text-left transition-all hover:bg-white/10"
        >
...
function WidgetCard({ loading, icon, iconBg, title, primary, secondary, tertiary, emptyTitle, emptySub, onClick }: WidgetCardProps) {
  return (
    <button
      onClick={onClick}
      className="group flex aspect-square flex-col justify-between rounded-2xl border border-white/10 bg-white/5 p-4 text-left transition-all hover:bg-white/10 hover:border-white/20"
    >
      ...
      {loading ? (
        <div className="h-8 w-full animate-shimmer rounded-lg" />
      ) : primary ? (
        <div className="min-w-0">
          <p className="truncate text-sm font-bold leading-tight">{primary}</p>
          {secondary && (
        ...
        <div className="flex flex-col items-start gap-1">
          <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-dashed border-white/20">
            <Plus className="h-4 w-4 text-muted-foreground" />
          </div>
          <p className="text-xs font-semibold text-muted-foreground">{emptyTitle}</p>
